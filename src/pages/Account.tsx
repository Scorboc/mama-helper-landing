...
export default function Account(){
  ...
  return <main className="account-page"><Link className="back-link" to="/"><ArrowLeft size={18}/> На главную</Link><div className="account-card tile">
    ...
    <form onSubmit={submit} className="space-y-5 mt-6"><label className="form-field">Электронная почта<Input type={mode==='login'?'text':'email'} inputMode={mode==='login'?'text':'email'} autoComplete="email" required maxLength={254} value={email} onChange={e=>setEmail(e.target.value)} placeholder={mode==='login'?'Почта или 1':'you@example.ru'}/></label>
    ...
    <label className="form-field">{mode==='recover'?'Новый пароль':'Пароль'}<div className="password-field"><Input type={visible?'text':'password'} required minLength={mode==='login'?1:12} maxLength={128} autoComplete={mode==='login'?'current-password':'new-password'} value={password} onChange={e=>setPassword(e.target.value)}/><button type="button" onClick={()=>setVisible(v=>!v)} aria-label={visible?'Скрыть пароль':'Показать пароль'}>{visible?<EyeOff size={19}/>:<Eye size={19}/>}</button></div>{mode!=='login'&&<span className="muted text-sm">От 12 символов. Можно использовать длинную фразу.</span>}</label>
    ...
    {mode==='login'&&<p className="text-center text-sm mt-5 text-muted-foreground">Тестовый вход: <strong>1</strong> · пароль: <strong>1</strong></p>}<p className="muted mt-4 text-sm flex gap-2"><Heart size={18} className="shrink-0"/>Профиль, переписка и карта наблюдений защищённо сохраняются на сервере. У каждого родителя — отдельный аккаунт.</p></>}
  </div></main>;
}
