...
export default function Account(){
  ...
  return <main className="account-page"><Link className="back-link" to="/"><ArrowLeft size={18}/> На главную</Link><div className="account-card tile">
    ...
    <form onSubmit={submit} className="space-y-5 mt-6"><label className="form-field">Электронная почта<Input type={mode==='login'?'text':'email'} inputMode={mode==='login'?'text':'email'} autoComplete="email" required maxLength={254} value={email} onChange={e=>setEmail(e.target.value)} placeholder={mode==='login'?'Почта или логин':'you@example.ru'}/></label>
    ...
    <p className="muted mt-4 text-sm flex gap-2"><Heart size={18} className="shrink-0"/>Профиль, переписка и карта наблюдений защищённо сохраняются на сервере. У каждого родителя — отдельный аккаунт.</p></>}
  </div></main>;
}
