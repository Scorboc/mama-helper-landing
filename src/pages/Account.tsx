...
export default function Account(){
  ...
  async function submit(e:FormEvent){e.preventDefault();if(busy)return;setBusy(true);setError('');
    try{const result=await api<Session>(mode,{email,password,consent,recoveryCode:recovery});setPassword('');setRecovery('');
  ...
  return <main className="account-page"><Link className="back-link" to="/"><ArrowLeft size={18}/> На главную</Link><div className="account-card tile">
    ...
    <form onSubmit={submit} className="space-y-5 mt-6"><label className="form-field">Электронная почта<Input type="email" autoComplete="email" required maxLength={254} value={email} onChange={e=>setEmail(e.target.value)}/></label>
    ...
    <label className="form-field">{mode==='recover'?'Новый пароль':'Пароль'}<div className="password-field"><Input type={visible?'text':'password'} required minLength={12} maxLength={128} autoComplete={mode==='login'?'current-password':'new-password'} value={password} onChange={e=>setPassword(e.target.value)}/><button type="button" onClick={()=>setVisible(v=>!v)} aria-label={visible?'Скрыть пароль':'Показать пароль'}>{visible?<EyeOff size={19}/>:<Eye size={19}/>}</button></div><span className="muted text-sm">От 12 символов. Можно использовать длинную фразу.</span></label>
    ...
    <p className="text-center mt-5"><Link to="/demo" className="text-primary underline">Посмотреть демо без регистрации</Link></p><p className="muted mt-4 text-sm flex gap-2"><Heart size={18} className="shrink-0"/>У каждого свой аккаунт, даже если вы родители одного малыша.</p></>}
  </div></main>;
}
