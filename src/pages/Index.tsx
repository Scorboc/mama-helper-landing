...
import { Bell, Heart, MessageCircle, Send, ShieldCheck, UserRound, BookOpen } from 'lucide-react';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from '@/components/ui/select';
import { assistant, Profile, Reply, sources } from '@/lib/assistant';
...
export default function Index() {
  ...
  return <div className="parent-app">
    ...
    <div className="workspace"><aside className="intro"><span className="eyebrow">ДЛЯ МАМ И ПАП</span><h1>Вы не одни.<br />Давайте по шагам.</h1><p>От ожидания малыша до первых трёх лет. Поддержка, полезные ориентиры и место для ваших вопросов.</p><div className="profile-summary"><UserRound /><div><strong>{profile.role}</strong><p>{context}</p></div></div><div className="safety-note"><ShieldCheck /><p>Помощник не заменяет врача. При угрозе жизни — <a href="tel:112">112</a>. Демо не умеет надёжно распознавать опасные состояния.</p></div><p className="small">Не вводите ФИО, контакты и медицинские документы. Данные этой страницы исчезнут после перезагрузки.</p></aside>
      <main className="main-panel"><Tabs value={tab} onValueChange={setTab}><TabsList className="app-tabs"><TabsTrigger value="chat"><MessageCircle size={18} />Чат</TabsTrigger><TabsTrigger value="advisor"><Bell size={18} />Советник</TabsTrigger><TabsTrigger value="profile"><UserRound size={18} />Профиль</TabsTrigger><TabsTrigger value="sources"><BookOpen size={18} />Источники</TabsTrigger></TabsList>
        ...
        <TabsContent value="sources"><div className="panel-heading"><div><h2>На что мы опираемся</h2><p>Открытые источники вместо ответов с форумов</p></div></div><div className="section-body"><article className="advice-card"><h3>Как справляться со стрессом</h3><p>ВОЗ · Doing What Matters in Times of Stress · 2020</p><a href={sources.stress} target="_blank" rel="noreferrer">Открыть руководство ↗</a></article><article className="advice-card"><h3>Прикорм в возрасте 6–23 месяцев</h3><p>ВОЗ · Guideline for complementary feeding · 2023</p><a href={sources.feeding} target="_blank" rel="noreferrer">Открыть рекомендации ↗</a></article><p>Ссылки проверены 6 сентября 2026 года. Это небольшая демонстрационная подборка, а не полная клиническая база. Медицинская экспертиза проекта ещё не проведена.</p><p>AI не может единолично гарантировать медицинскую достоверность. До запуска персональных рекомендаций потребуются клиническая проверка содержания и тестирование безопасности.</p></div></TabsContent>
