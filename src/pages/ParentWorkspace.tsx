  ...
  return <div className="parent-workspace"><header className="cabinet-header"><Link to="/" className="flex items-center gap-2"><img src="/logo-mark.png" alt="" className="h-12"/><strong>Мамин помощник</strong></Link><div className="flex items-center gap-3"><span className="test-pill">Тест без оплаты</span><Button variant="ghost" onClick={()=>void logout()} disabled={busy} aria-label="Выйти"><LogOut size={18}/></Button></div></header>
    ...
    <TabsContent value="chat"><section className="tile chat-card">
      ...
      <div className="chat-log" role="log" aria-label="Переписка" aria-live="polite">
        {!state.messages.length&&<div className="bubble assistant"><span>Мамин помощник</span><p>Помогу обсудить сон, кормление, игры, уход и то, как вы сами себя чувствуете. Маме и папе тоже нужна поддержка. Для советов по возрасту заполните профиль. Диагнозы и назначения — к врачу.</p></div>}
