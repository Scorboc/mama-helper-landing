...
function inScope(q, state) {
  ...
  if (/школ|дошколь|детск.*сад|садик|собрат.*(сад|садик)|что.*взять.*(сад|садик)|одежд.*(сад|садик)|утренн.*сбор|адаптац.*сад|букв|чтен|читать|читал|читали|книг|сч[её]т|считал|считали|цифр|математ|письм|карандаш|логопед|самостоятель|внимани|памят|усидчив|друж|ссор|делиться|буллинг|готовност/i.test(text)) return true;
  if (
