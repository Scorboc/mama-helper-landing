...
function inScope(q, state) {
  ...
  if (/^(привет|здравствуй|добрый (день|вечер|утро)|что ты умеешь)[!.? ]*$/i.test(text))
    return true;
  if (/школ|дошколь|детск.*сад|садик|букв|чтен|читать|сч[её]т|цифр|математ|письм|карандаш|логопед|самостоятель|внимани|памят|усидчив|друж|ссор|делиться|буллинг|готовност/i.test(text)) return true;
  if (
...
const SCOPE_BOUNDARY =
  "Я помогаю по вопросам беременности, поддержки мам и пап, ухода и развития детей до 6 лет включительно: детский сад, общение, игры и подготовка к школе. С автомобилями, BMW, программированием и другими посторонними темами я не работаю.";
