...
CREATE TABLE IF NOT EXISTS mh_state (
 user_id TEXT PRIMARY KEY, encrypted_data TEXT NOT NULL, revision BIGINT NOT NULL DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS mh_push (
 user_id TEXT NOT NULL, endpoint_hash TEXT PRIMARY KEY, encrypted_subscription TEXT NOT NULL
);
CREATE INDEX IF NOT EXISTS mh_push_user ON mh_push(user_id);
CREATE TABLE IF NOT EXISTS mh_delivery (
 user_id TEXT NOT NULL, event_id TEXT NOT NULL, endpoint_hash TEXT NOT NULL,
 sent_at BIGINT NOT NULL, PRIMARY KEY(user_id,event_id,endpoint_hash)
);